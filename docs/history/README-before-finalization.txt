<!-- 현재 상태 요약 -->
> **현재: M4 품질·운영 검증 진행 중.** M3 자연어 초안 → 사람 승인 → 티켓 생성과 실제 재시작 후 승인 대기 복원을 확인했습니다. 초안 프롬프트 v2 적용 후 실제 API 회귀 8/8 및 개발 Agent의 문장 검토 8건 적합을 확인했습니다. 최초 요청 지연 실패는 별도 기록했으며 미사용 질문·운영 검증은 남아 있습니다. [M4 진행 현황](docs/m4-status.md)과 [남은 작업](docs/remaining-work.md)을 참고하세요.

# 기업 업무 대응 AI Agent

사내 운영 문서와 장애 데이터를 함께 조회하고, 근거 있는 대응 방안을 제시하며, **사람의 승인 후 후속 티켓을 생성**하는 업무 지원 서비스입니다.

> Streamlit → FastAPI → Spring Boot → PostgreSQL 조회, 실제 RAG·LangGraph·생성 답변과 승인·티켓 화면을 연결했습니다. 자연어 기간 필터 추출, 실제 응답 유실 복구 검증 및 성능 측정은 남아 있습니다. 아래 전체 요구사항과 성능 수치는 최종 목표입니다.
